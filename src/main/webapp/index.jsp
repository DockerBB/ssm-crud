	...
	var current_page;//当前页
		...
		function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var editBtn = $("<button></button>").addClass(
						...
						$("<span></span>").addClass("glyphicon-pencil"))
						.append("编辑");
				 //位编辑按钮添加自定义属性表示当前员工ID
				 editBtn.attr("edit-id",item.empId);
				var delBtn = $("<button></button>").addClass(
						"btn btn-danger btn-sm delet_btn").append(
						$("<span></span>")
		...
		function validate_add_form(){
			...
			if(!regName.test(empName)){
				//alert("用户名可以是2-5位中文或6-16位英文");
				show_validate_msg("#empName_add_input","error","用户名可以是2-5位中文或6-16位英文的组合");
				return false;
			}else{
